...
const Base = require('../base/base.js'); // 假设Base类在../base/base.js中
const { gdir, env } = require('../globalvars.js'); // 假设globalvars.js在../globalvars.js中
let eggSocket = null, eggSocketServer = null;
let { encyclopedia } = require('../globalvars.js'); // 假设encyclopedia在../globalvars.js中
...
class Httptool extends Base {
    ...
    //#TODO 1 添加一个方法 .testNetByNetworkAddress，输入一个域名或IP地址，测试地址是否能连网，默认超时时间2秒
    async testNetByNetworkAddress(address, timeout = 2000) {
        return new Promise((resolve) => {
            const socket = net.createConnection({ host: address, port: 80 }, () => {
                // 连接成功
                socket.end();
            ...
            socket.setTimeout(timeout, () => {
                socket.end();
                resolve({ success: false, message: `连接超时: ${timeout}ms` });
            });
        });
    }
